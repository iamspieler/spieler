...
class Breadcrumbs {
   ...
   static function generate($bred_array, $bc_lang = NULL){
      ...
      foreach($bred_array as $bred){
         ...
         if (!isset($bred['link']) || $i == count($bred_array))
			...
			$tr_slash = ($bred['link'] != URL::base('http').$bc_lang) ? "/" : "";
			$bred_generated[] = '<a href="'.$bred['link'].$tr_slash.'">'.HTML::chars($bred['name']).'</a>';
		}
      }
      $bred_generated = implode('<span class="separator"> '. Kohana::$config->load('global.site_bc_sep') .' </span>', $bred_generated);
