...
class Controller_Admin_Articles extends Controller_Admin {
    ...
    */
    public function action_index()
    {
	    ...
	    $content = View::factory('/Admin/'.$this->c_controller.'/List')
		       ...
		       ->bind('articles_list', $articles_list)
		       ->bind('sections_list_short', $articles_list_sections)
		...
		$count_c = Model::factory('Articles')->ap_count_list($group_active);
	    ...
	    $articles_list = Model::factory('Articles')->ap_get_list($cur_page, $group_active);
	    $articles_list_sections = Model::factory('Articles')->ap_get_sections_short($group_active);
    ...
    */
    public function action_sections()
    {
	 ...
	 $sections_list = Model::factory('Articles')->ap_get_sections();
    ...
    */
    public function action_issues()
    {
	  ...
	  $valid -> rule('group_id', 'not_empty')
		 -> rule('group_id', 'max_length', array(':value', 11))
	         -> rule('group_id', 'digit');
	  $group_active = (($valid -> check()) and ($_GET['group_id'] != 'null')) ? $_GET['group_id'] : NULL;
	  
	  print_r($_GET);

	  $content = View::factory('/Admin/'.$this->c_controller.'/Issues_List')
		   ->bind('breadcrumbs', $breadcrumbs)
		   ->bind('issues_list', $issues_list)
		   ->bind('editions_list_short', $issues_list_editions)
		   ->bind('group_active', $group_active)
		   ->bind('main_menu', $main_menu);
	  ...
	  $count_c = Model::factory('Articles')->ap_count_issues($group_active);
	  $total_items = $count_c[0]['total'];

	  $content->pagination = Pagination::factory(
			       array(
						'total_items' => $total_items,
						'items_per_page' => Kohana::$config->load($this->clow_controller.'.mod_ap_i_by_page')
			       )
	  );
	 ...
	 $issues_list = Model::factory('Articles')->ap_get_issues($cur_page, $group_active);
	 $issues_list_editions = Model::factory('Articles')->ap_get_editions_list($group_active);
     ...
     * 
    */
    public function action_articles_form()
    {
	     ...
	     $news_list = array(
			...
			'article_picture' => '',
			'article_picture_dir' => '',
			'article_issue_date' => date('d.m.Y H:i:s'),
			'article_issue_hour' => date('H'),
			'article_issue_minute' => date('i'),
			'article_publish_date' => date('d.m.Y H:i:s'),
			'article_publish_hour' => date('H'),
			'article_publish_minute' => date('i'),
			'article_unpublish_date' => '0000-00-00 00:00:00',
			'article_unpublish_hour' => '00',
			'article_unpublish_minute' => '00',
			'article_main' => '',
			'article_status' => '',
			'id_section' => '',
	       );    
	       ...
	       if(HTML::chars($this->request->param('act')) == 'edit') {
		    ...
		    $validate_param -> rule('act', 'not_empty')
			      ...
			      if($validate_param -> check()) {   
				    $article_id = $validate_param['id']; // id записи для извлечения из базы
				    $action = $validate_param['act'];	 // определяяем action - добавление или редактирование
				    

				    // если определен id записи и не равен нулю
				    if($article_id != 0) {
					 $articles_list = Model::factory('Articles')->ap_get_profile($article_id); // выборка контакта из БД
					 
					 $users_list = Model::factory('Users')->get_list_short(); // выборка контакта из БД
	
					 $activity_flag = ($articles_list['article_status'] != 0) ? TRUE : FALSE;   // определяем активность в форме
					 $ismain_flag = ($articles_list['article_main'] != 0) ? TRUE : FALSE;   // главная новость или нет
					 
					 $article_issue = $articles_list['issue_id'];
					 $articles_issues_list = Model::factory('Articles')->ap_get_issues_list();
					 
					 $url_change = TRUE;   // определяем активность в форме
					 
					 // заголовок, action и subaction формы
					 $form_title = __('Articles_edit');
					 $form_action = 'post';
	       ...
	       */
	       elseif(HTML::chars($this->request->param('act')) == 'add') {
		    ...
		    $validate_param -> rule('act', 'not_empty')
			      ...
			      if($validate_param -> check()) {
				    ...
				    $article_issues = 0;
				    $articles_issues_list = Model::factory('Articles')->ap_get_issues_list_year();
				    $articles_sections_list = Model::factory('Articles')->ap_get_sections_list();
	       ...
	       $content = View::factory('/Admin/'.$this->c_controller.'/Form')
		       ...
		       ->bind('articles_issues_list', $articles_issues_list)
		       ->bind('articles_sections', $articles_sections_list)
